defmodule BinanceMock do
  ...
  defp order_limit(symbol, quantity, price, side) do
    %Binance.Order{} = 
      fake_order = 
      generate_fake_order(symbol, quantity, price, side)
  ...
  def handle_cast(
...

    {
      :noreply,
      %{
        state | order_books: updated_order_books,
  ...
  defp generate_fake_order(symbol, quantity, price, side)
    when is_binary(symbol) and is_binary(quantity) and is_binary(price) and (side == "BUY" or side == "SELL") do
  ...
  def handle_info(%MarketTrade{} = trade_event, %{order_books: order_books} = state) do
    ...
    order_books =
      Map.replace!(
        ...
        %{
          ...
          historical:
            filled_buy_orders ++ filled_sell_orders ++ order_book.historical
        }
      )
  ...
  defp broadcast_trade_event(%MarketTrade{} = trade_event) do
