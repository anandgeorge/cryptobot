...
#	end
	...
	def handle_continue(:start_traders, %{symbol: symbol} = state) do
		...
		Logger.info("Start traders for #{symbol}")
	...
	def handle_call(
		...
		case Enum.find_index(traders, &(&1.pid == trader_pid)) do
			nil -> Logger.warn("Tried to update the state of trader that leader is not aware of")
				{:reply, :ok, state}
			index ->
	...
	def handle_info(
		...
		Logger.info("#{symbol} trader finishing trading - restarting")
